...
#define TSS_AND_SUM_AVALIABLE               (NEUTON_DSP_STAT_CTX_TSS_BIT | NEUTON_DSP_STAT_CTX_SUM_BIT)
#define ONLY_TSS_AVALIABLE                  NEUTON_DSP_STAT_CTX_TSS_BIT
#define ONLY_SUM_AVALIABLE                  NEUTON_DSP_STAT_CTX_SUM_BIT

// ///////////////////////////////////////////////////////////////////////////

static neuton_i16_t var_by_sum_n_tss_(neuton_i32_t sum, neuton_u32_t sum_of_squares,
                                     neuton_u16_t num)
{
...
static neuton_i16_t var_by_mean_(const neuton_i8_t* p_input, neuton_u16_t num, 
...
static neuton_i16_t var_by_mean_s_(const neuton_i8_t* p_input, neuton_u16_t num, 
                                    size_t stride, const neuton_i8_t mean)
{
    neuton_u16_t loop_cnt;
    neuton_u16_t i = 0;
    neuton_i16_t diff;
                                ...
                                neuton_dsp_stat_ctx_i8_t* p_ctx)
{
    ...
    if (p_ctx)
    {
        ...
        case TSS_AND_SUM_AVALIABLE:
            var = var_by_sum_n_tss_(p_ctx->value.sum, p_ctx->value.tss, num);
            break;
        case ONLY_TSS_AVALIABLE:
            var = var_by_sum_n_tss_(neuton_dsp_sum_i8(p_input, num, p_ctx), p_ctx->value.tss, num);
            break;
        case ONLY_SUM_AVALIABLE:
            var = var_by_mean_(p_input, num, p_ctx->value.sum / num);
            break;
        default:
    ...
    else
    {
        ...
        var = var_by_sum_n_tss_(ctx.value.sum, ctx.value.tss, num);
    }
                                ...
                                size_t stride, neuton_dsp_stat_ctx_i8_t* p_ctx)
{
    ...
    if (p_ctx)
    {
        ...
        case TSS_AND_SUM_AVALIABLE:
            ...
            break;
        case ONLY_TSS_AVALIABLE:
            var = var_by_sum_n_tss_(neuton_dsp_sum_i8_s(p_input, num, stride, p_ctx), p_ctx->value.tss, num);
            break;
        case ONLY_SUM_AVALIABLE:
            var = var_by_mean_s_(p_input, num, stride, p_ctx->value.sum / num);
            break;
        default:

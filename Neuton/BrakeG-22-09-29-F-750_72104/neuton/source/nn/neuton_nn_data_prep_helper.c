...
#include <neuton_generated/neuton_user_data_prep_meta.inc>
                                ...
                                neuton_u8_t args_num, void* p_args)
{
    ...
    if ((ppln == NEUTON_DSP_PIPELINE_SPECTRAL) && (feature_id == NEUTON_DSP_SPECTR_FID_SPECTRUM))
    {
        ...
        neuton_dsp_rfht_init(p_args, DSP_FHT_SIN_TABLE, DSP_FHT_TANG_TABLE, DSP_FHT_REV_BIT_INDEX_TABLE, 
                            NULL, DSP_FHT_WINDOW_SIZE, DSP_FHT_LOG_N);
        return;
    }
#endif
                                                ...
                                                neuton_feature_t* p_features)
{
    NEUTON_NN_LOG_DEBUG("Extract features for %d uniq inputs from window with %d samples\r\n", 
                        p_ctx->uniq_features_collected, p_ctx->max_samples_num);
    ...
    const neuton_u16_t uniq_features_num = p_ctx->uniq_features_collected;
    const neuton_u16_t samples_num       = p_ctx->max_samples_num;
    ...
    for (neuton_u16_t i = 0; i < uniq_features_num; i++)
    {
        ...
        extracted = funcwrapper_extract_features(p_input_window, samples_num, mask, 
                                                    get_argument_fn, p_features);
        p_features += extracted;
        p_input_window += samples_num;
...
{
    const neuton_u16_t uniq_feature_samples_num = p_ctx->max_samples_num;
    const neuton_u16_t uniq_features_collected = p_ctx->uniq_features_collected;
    ...
    const neuton_u16_t window_samples_num = (uniq_features_collected * uniq_feature_samples_num);
    /** Set pointers to the end of the input window */
    p_scaled = ((neuton_model_input_t *)p_input_window) + window_samples_num;
    p_input_window += window_samples_num;

    for (neuton_i16_t i = uniq_features_collected - 1; i >= 0; i--)
    {
        ...
        p_input_window -= uniq_feature_samples_num;
        p_scaled -= uniq_feature_samples_num;
    }
#else
    p_scaled = (neuton_model_input_t*)p_input_window;

    for (neuton_u16_t i = 0; i < uniq_features_collected; i++)
    {
        if (USE_LAG_FROM_INPUT_FEATURE(i))
        {
#if NEUTON_NN_USE_UNIQ_INPUT_SCALING
            ...
            max = INPUT_FEATURES_SCALE_MAX[i];
#endif
            p_lag_features = p_input_window + (uniq_feature_samples_num - lag_features_num);

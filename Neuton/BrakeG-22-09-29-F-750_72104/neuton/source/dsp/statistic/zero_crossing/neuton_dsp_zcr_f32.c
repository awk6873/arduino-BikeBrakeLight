...
neuton_f32_t neuton_dsp_zcr_f32(const neuton_f32_t *p_input, neuton_u16_t num)
{
    ...
    sign_val = NEUTON_F32_CHECK_SIGN(*p_input++);

    while (loop_cnt > 0U)
    {
        sign_next_val = NEUTON_F32_CHECK_SIGN(*p_input++);
                                ...
                                size_t stride)
{
    ...
    sign_val = NEUTON_F32_CHECK_SIGN(p_input[i * stride]);
    i++;

    while (loop_cnt > 0U)
    {
        sign_next_val = NEUTON_F32_CHECK_SIGN(p_input[i * stride]);

...
typedef neuton_i8_t fft_fixed_t;
typedef neuton_i16_t fft_fixed2_t;
typedef neuton_u16_t fft_count_t;

#define FIXED_MULT(x, y) (((fft_fixed2_t)(x) * (y)) >> NEUTON_FRACT_BITS_7)
#define FIXED_DIV(x, y) (((fft_fixed2_t)(x) << NEUTON_FRACT_BITS_7) / (y))
#define GET_OVERFLOW_MARKER(x) (x > NEUTON_INT8_MAX || x < NEUTON_INT8_MIN)
...
{
    neuton_i8_t* p_data = (neuton_i8_t *)p_inst->p_window;
    const neuton_i8_t* p_sin_table = (const neuton_i8_t*)p_inst->p_sin_table;
    const neuton_i8_t* p_tg_table = (const neuton_i8_t*)p_inst->p_tg_table;
...
for (fft_count_t i = 0; i < window_size; ++i)
    ...
    fft_fixed2_t inc = 0;
    if (p_data[0] & ((fft_fixed_t)1U << (NEUTON_FRACT_BITS_7 - 1)))
    {
        inc = 1;
        ++ex;
    }

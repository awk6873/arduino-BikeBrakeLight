#include <neuton/dsp/pipeline/neuton_dsp_pipeline_spectr.h>
#include <neuton/dsp/pipeline/config/neuton_dsp_pipeline_config_i8.h>
...
#define UPSCALE_TO_I16(peak, fraction_bit)      ((neuton_i16_t)(peak) << (NEUTON_FRACT_BITS_7 - fraction_bit))

// ///////////////////////////////////////////////////////////////////////////

neuton_u16_t neuton_dsp_pipeline_spectr_compute_i8(const neuton_i8_t* p_input, neuton_u16_t num,
                                                ...
                                                neuton_i16_t* p_features)
{
    ...
    neuton_u16_t fraction_bits = neuton_dsp_rfht_i8(&rfht);  
...
// ///////////////////////////////////////////////////////////////////////////
    ...
    if (FEATURES_USED_PEAKS(feature_mask))
    {
        ...
        neuton_dsp_findpeaks_i8((neuton_i8_t*)rfht.p_window, rfht.window_size / 2, 
                                peaks_index, NULL);

#if (NEUTON_DSP_PIPELINE_COMPUTE_SPECTR_PEAKS_FREQ)
        if (feature_mask.is.peaks_freq)
        {
            neuton_i16_t usage_mask;
        ...
        {
            neuton_i16_t usage_mask;
            const neuton_i8_t* p_spectrum = (const neuton_i8_t*)rfht.p_window;
            get_argument(NEUTON_DSP_PIPELINE_SPECTRAL, NEUTON_DSP_SPECTR_FID_PEAKS_AMPL, 1, &usage_mask);

            for (size_t i = 0; i < NEUTON_FIND_PEAKS_MAX_NUM; i++)
            {
                if (IS_PEAK_USED(usage_mask, i))
                    *p_features++ = UPSCALE_TO_I16(p_spectrum[peaks_index[i]], fraction_bits);
            }
        }
#endif
    }

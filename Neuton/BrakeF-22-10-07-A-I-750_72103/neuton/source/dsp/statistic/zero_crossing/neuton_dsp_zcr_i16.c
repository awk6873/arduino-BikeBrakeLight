...
neuton_i16_t neuton_dsp_zcr_i16(const neuton_i16_t *p_input, neuton_u16_t num)
{
    ...
    sign_next_val = NEUTON_INT16_CHECK_SIGN(*p_input);
    ...
    while (loop_cnt > 0U)
    {
        ...
        sign_val = NEUTON_INT16_CHECK_SIGN((i16x2 << 16U) >> 16U);

        if (sign_val != sign_next_val)
            zcr++;

        sign_next_val = NEUTON_INT16_CHECK_SIGN(i16x2 >> 16U);
    ...
    loop_cnt = num - 1;
    sign_val = NEUTON_INT16_CHECK_SIGN(*p_input++);
                                ...
                                size_t stride)
{
    ...
    sign_val = NEUTON_INT16_CHECK_SIGN(p_input[i * stride]);

    while (loop_cnt > 0U)
    {
        sign_next_val = NEUTON_INT16_CHECK_SIGN(p_input[i * stride]);
